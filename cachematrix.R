#The funtion inv_matrix() takes a matrix in the current environement and assigns it into another environmet for storage
...
# Function 'cached_matrix' gets an inverse of a matirx 'x$getinverse' from a list if it exists if not, 
# Calculates the inverse 'inverse2' from the new matix supplied
cached_matrix <- function(x = matrix()) {
                m <- x$getinverse()
                if (!is.null(m)) {
